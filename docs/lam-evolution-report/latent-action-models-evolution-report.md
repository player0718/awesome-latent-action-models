...
Review note：本文是基于本仓库 README 的 snapshot research note，不是系统综述。部分 2026 工作仍是新近 preprint 或系统报告；图像是概念示意，不代表论文原图、实验结果或严格谱系树。

## 摘要

Latent Action Model (LAM) 研究的核心问题不是“如何直接预测机器人关节动作”，而是：能否从相邻状态、视频片段或无动作标注轨迹中学习一个紧凑的、可复用的“变化原因”表示，并把它作为后续世界模型、策略学习、VLA 预训练或跨 embodiment 迁移的控制接口。相邻文献中会出现 latent action、action token、motion latent、skill latent 或 world-action latent 等相关术语，但它们并不完全等价；本文关注的是从状态转移或视频变化中学习可控 latent，并能被 grounding、prediction 或 policy interface 复用的那一部分。

从历史上看，LAM 不是从 World Action Model (WAM) 单线衍生出来的。更准确的谱系是：2019 年前后的 observation-only imitation 和 latent policy 学习提出了“从状态转移中反推出动作”的基本问题；2021-2024 年 playable video 与 Genie 把离散 latent action 做成可交互世界模型的控制字母表；2024-2025 年 LAPO、LAPA、UniVLA、CLAM 等工作把这种表示带入机器人策略和 VLA 预训练讨论；2025-2026 年 AdaWorld、CoLA-World、DreamZero、FLAM、DiLA 等工作则让 LAM、world model、video diffusion、world-action joint modeling 在部分问题上发生交集。
...
**Figure 1. LAM Research Map.** LAM 位于中心，连接 observation-only learning、VLA pretraining、robot policies、world models、WAM 与 cross-embodiment transfer。图中 WAM 表示与 latent-action interface 发生重叠的相邻系统路线，而不是 LAM 的父类或严格子类。
...
| WAM / world-action model | 世界状态与动作的联合生成或预测模型 | 可有可无 | world-model control、zero-shot policy | 与部分 LAM 在 latent-action interface 上重叠；泛 WAM 不自动属于 LAM |

关键区别在于，LAM 关注的是“动作表示如何被学出来、是否真的捕捉了可控变化”；WAM 关注的是“世界状态与动作如何被联合建模并用于闭环策略”。一个 WAM 可以包含或复用 latent action，但不是所有 LAM 都是 WAM，也不是所有 WAM 都属于 LAM。

## 2. 历史路线图：2019-2026

LAM 的阅读路线可以分成五个阶段。这里的阶段是便于阅读的多泳道压缩，不代表所有工作之间存在直接继承关系。

![Figure 2: Historical Evolution of LAMs](figures/fig2-historical-evolution.png)

**Figure 2. Historical Evolution of LAMs.** 时间线从 observation-only imitation 和 playable video 出发，经过 Genie/LAPO 的 2024 年节点，在 2025 年分化到 VLA 与 world-model 两条强应用线，并在 2026 年出现结构化、层级化、factored、WAM 交集和评测化趋势。这是概念路线图，不是直接祖先关系图。
...
Genie 把 action-free video 中学到的离散 VQ latent actions 用作 generative interactive environment 的控制接口。它的影响在于证明：即便没有真实动作标签，也可以从大量视频中学出能驱动可交互世界模型的 action tokens。Genie 强化了 latent-action interface 在 foundation world model 语境中的可见度，但不是 LAM 的唯一来源。
...
第一条是 VLA / robot policy pretraining。LAPA 把 latent action prediction 做成 VLM/VLA 预训练目标：从视频中量化视觉变化为 latent action tokens，再训练模型预测这些 tokens，最后在动作标注机器人数据上 fine-tune。UniVLA、CLAM、Moto、GR00T N1、villa-X 等工作可以放入这条应用线讨论，但它们的数据来源、表示形式和 grounding 策略并不相同。

第二条是 world-model control。AdaWorld、CoLA-World、LAWM 等工作把 latent actions 放到世界模型中作为控制变量，让模型在 latent control 下预测未来状态或生成可控 rollouts。WorldVLA、DreamZero 等更接近 broader action-world / WAM 系统，应作为相邻路线讨论，而不是典型 action-free LAM 控制变量工作。这里的关键转变是，LAM 不再只为策略提供预训练标签，也可以直接成为部分 world model 的条件变量。
...
### 2.5 2026：结构化、WAM 交集、评测化与规模化

2026 年的 LAM 研究呈现四种趋势。
...
第二是与 WAM / action-world system 的交集。DreamZero 等 WAM 工作把 video/action joint modeling 与 zero-shot policy 结合，强调在世界模型或视频扩散模型中直接建模 action-conditioned dynamics。它们不是 LAM 的起点，也不自动是 LAM 子类；只有显式学习、复用或评估 latent-action interface 的部分才进入本文讨论范围。

第三是评测化。LARY、ALAM probes、LAOM/LAOF/Object-Centric LAM 等工作部分回应同一个问题：下游 policy success 不等于 latent action quality。一个 LAM 看起来有效，可能只是因为 world model 更大、policy head 更强、数据更多，而不一定说明 `z_t` 真正学到了可控动作。

第四是 scale、post-training 与 VLA latent-world integration。CLAP、LDA-1B、VLA-JEPA、World2Act、CoWVLA 等工作把 latent-action 目标接入更大规模的数据 ingestion、VLA 后训练或 latent world reasoning 中，说明 2026 的主线不只是 WAM 与结构化。
...
![Figure 4: From Passive Video to Candidate Policy](figures/fig4-passive-video-to-policy.png)

**Figure 4. From Passive Video to Candidate Policy.** LAM 的 practical promise 是把大规模 passive video 转换为可利用的动作先验，再用少量真实动作完成 grounding。该图是 schematic promise，不是经验结果；最终 policy 仍需要 grounding、评测和真实任务验证。
...
LAM 的表示形式正在从“单一离散 token”扩展成多个非排他技术视角。

![Figure 5: Technical Family Tree of LAMs](figures/fig5-technical-family-tree.png)

**Figure 5. Technical Family Tree of LAMs.** 这张图应读作 technical lenses / non-exclusive families，而不是严格分类树：一个系统可以同时是 continuous、hierarchical、world-model-conditioned，也可以在 discrete tokens 上加入 algebraic probes。World-Action 表示与 LAM 相邻的系统层交集。
...
| World-Action overlap | DreamZero, Latent-WAM | 世界状态与动作联合建模中显式接入 latent-action interface 的交集 | 适合讨论闭环 world-model control 与 LAM 的关系 | 容易把 LAM quality 与 world-model capacity 混在一起；泛 WAM 不自动属于 LAM |

## 6. 精选核心矩阵

年份按主会议、系统发布时间或本仓库 README 的 publication-year 口径；arXiv 首发年份见参考链接。该表是精选矩阵，不是完整 bibliography；`scope role` 用来区分核心 LAM、系统交集、benchmark 和相邻路线。

| 年份 | 论文/系统 | scope role | 路线位置 | latent action 形式 | 主要贡献 |
|---:|---|---|---|---|---|
| 2019 | ILPO | Core precursor | observation-only imitation | discrete | 从 state-only observations 推断 latent actions，并用少量交互 grounding |
| 2019 | CLASP | Core precursor | passive video -> control | continuous | 从被动视频学习可组合 latent action space |
| 2021 | Playable Video Generation | Core precursor | playable video | discrete | 从无标签视频学习可控制视频的动作词表 |
| 2024 | Genie | Core system | interactive world model | discrete VQ | 把 VQ latent actions 作为生成世界模型的控制接口 |
| 2024 | LAPO | Core LAM | policy grounding | continuous -> discrete | observation-only videos 到 policy 的最小闭环 |
| 2025 | LAPA | Core LAM | VLA pretraining | discrete VQ | 用 latent action prediction 作为 VLA 预训练目标 |
| 2025 | UniVLA | Core LAM | VLA/cross-embodiment | discrete VQ | task-centric latent actions，弱化 embodiment 和 camera nuisance |
| 2025 | CLAM | Core LAM | robot learning | continuous | 从 unlabeled demonstrations 学连续 latent actions 并解码到策略 |
| 2025 | LAWM | Core LAM/WM | VLA/world model | continuous | 把 latent action 作为 world-model conditioning 变量用于预训练 |
| 2025 | CoMo | Core LAM | VLA pretraining | continuous motion | 从互联网视频学习 continuous latent motion，并加入 contrastive/TD 约束 |
| 2025 | AdaWorld | Core LAM/WM | world model | continuous | 用 latent actions 预训练 adaptable world models |
| 2025/2026 | CoLA-World | Core LAM/WM | joint LAM + world model | continuous | 从头训练 LAM 并与 pretrained video world model 协同 |
| 2026 | HiLAM | Core LAM | hierarchy | hierarchical | 学短时 motion 与长时 skill latent |
| 2026 | ALAM | Core LAM/probing | structured/probing | continuous structured | 用加法、可逆性等代数一致性约束和 probe LAM |
| 2026 | FLAM | Core LAM/WM | factorization | factored | 多实体动态中的 factored latent action |
| 2026 | DiLA | Core LAM/WM | disentanglement | continuous | disentangled latent action world model 与 cycle-transfer metric |
| 2026 | LARY | Benchmark | evaluation | evaluation | 面向 latent action representations 的专门 benchmark |
| 2026 | DreamZero | Related WAM system | action-world joint modeling | explicit action-video joint modeling | 把 world action model 用作 zero-shot policy；与 LAM 在 action/world interface 上相邻 |
...
对需要规划或闭环控制的 world model，如果缺少可干预的 action/latent-action 条件变量，预测容易退化为不可控的视频续写。LAM 给部分 world model 提供可控条件变量：同一初始状态下，不同 latent action 应该导向不同未来。这就是 Genie、AdaWorld、CoLA-World、LAWM 等工作的共同动机。

但这也带来评估混淆：world model 生成得好，不一定说明 latent action 好；可能只是模型容量大、数据强、decoder 会补细节。

### 7.3 LAM 与 WAM

WAM 与 LAM 的关系更适合理解为相邻交集：WAM 不仅学世界状态，也显式把 action 或 action-like variable 纳入联合建模，并希望模型在闭环中直接产生可执行行为；LAM 则关注 latent-action representation/interface 本身。DreamZero 的“world action model as zero-shot policy”代表了系统层 action-world modeling 方向，但不等同于核心 LAM taxonomy。

所以，LAM 与部分 WAM 在 latent-action interface 上重叠；WAM 不是 LAM 的历史来源，也不自动是 LAM 的子类。
...
普通 video prediction 只要求预测可能的未来；LAM 要求找到能区分不同未来的控制变量。Genie 强化了一个重要方向：latent action 不再只是辅助表示，也可以成为可交互生成世界的控制字母表。
...
**Figure 7. Future Directions for LAMs.** 当前趋势显示，LAM 研究可能继续从“能不能从视频中学出动作”转向“这个 latent interface 是否可组合、可迁移、可 grounding、可控制、可测量”。图中只展示 interface-quality 方向；data curation 作为配套问题在下表列出。
...
如果目标是快速理解 LAM 的来龙去脉，建议按以下路线阅读。表中的 `->` 是阅读顺序，不代表历史因果顺序。
...
LAM 的主线不是某个单一模型家族，而是一种研究问题：从无动作或弱动作数据中学习可复用的动作变化表示。这个问题早期来自 observation-only imitation 和 passive video control，并行地被 Genie 式 interactive world model、LAPA/UniVLA 式 VLA pretraining、CoLA-World/FLAM/DiLA 式 latent-action world modeling、ALAM/HiLAM 式结构化表示和 LARY/LAOM 式评测分析共同推进。

如果用一句话概括进化轨迹：

> LAM 正在从“把视频变化压成一个 token”进化为“为 embodied foundation models 提供可组合、可迁移、可 grounding、可控制、可评测的动作接口”。

这个判断也解释了为什么 LAM 与 WAM 关系紧密但不等价：二者在部分 action/world interface 上重叠，但 LAM 是 latent-action representation/interface 问题，WAM 是更广义的 world-action 系统建模方向。
...
- LAWM: Latent Action Pretraining Through World Modeling. https://arxiv.org/abs/2509.18428
- LatBot: Distilling Universal Latent Actions for Vision-Language-Action Models. https://arxiv.org/abs/2511.23034
...
- CoWVLA: Chain of World: World Model Thinking in Latent Motion. https://arxiv.org/abs/2603.03195
- World2Act: Latent Action Post-Training from World Model Dynamics. https://arxiv.org/abs/2603.10422
...
- LAOF: Robust Latent Action Learning with Optical Flow Constraints. https://arxiv.org/abs/2511.16407
- Why Latent Actions Fail, and How to Prevent It. https://arxiv.org/abs/2605.20223
- XSkill: Cross Embodiment Skill Discovery. https://arxiv.org/abs/2307.09955
- LAC-WM: Latent Action Robot Foundation World Models for Cross-Embodiment Adaptation. https://openreview.net/forum?id=vEZgPr1deb
